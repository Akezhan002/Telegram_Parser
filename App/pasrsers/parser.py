...
for count in range(1, 8):
    url = requests.get(f"https://www.sulpak.kz/f/noutbuki?page={count}")
    ...
    for el in data:
        ...
        productHistory = laptop_service.find_one_by_name(nameR)
        if productHistory is None:
            if not (dis is None):
                for a in dis:
                    s1 = "".join(c for c in a if c.isdecimal())
                    laptop_service = LaptopService()
                    laptop_service.create_laptop(nameR, res, descR[0], showCase, s1, photo)
            elif not (not(dis is None)):
                laptop_service = LaptopService()
                laptop_service.create_laptop(nameR, res, descR[0], showCase, 0, photo)
        else:
            print('parser')
